...
const ACCENTS = {
  brass:       { text: 'text-brass',       bg: 'bg-brass/10',       border: 'border-brass/20',       hex: '176,141,87' },
  mint:        { text: 'text-mint',        bg: 'bg-mint/10',        border: 'border-mint/20',        hex: '126,184,159' },
  crimson:     { text: 'text-red-400',     bg: 'bg-crimson/10',     border: 'border-crimson/20',     hex: '140,47,57' },
  blue:        { text: 'text-blue-400',    bg: 'bg-blue-500/10',    border: 'border-blue-500/20',    hex: '59,130,246' },
  gold:        { text: 'text-gold',        bg: 'bg-gold/10',        border: 'border-gold/20',        hex: '201,169,97' },
  champagne:   { text: 'text-champagne',   bg: 'bg-champagne/10',   border: 'border-champagne/20',   hex: '212,185,150' },
...
export default function PremiumStatCard({ label, value, sublabel, icon: Icon, accent = 'brass', alert, alertIcon: AlertIcon, onClick, hero, className = '' }) {
  ...
  return (
    <Tag
      ...
      style={hero ? {
        background: `linear-gradient(135deg, rgba(${a.hex}, 0.1) 0%, rgba(30,39,53,0.4) 50%, rgba(14,26,43,0.2) 100%)`,
      } : undefined}
    >
      ...
      {sublabel && <p className="text-[#AAB4C3]/60 text-[10px] mt-0.5">{sublabel}</p>}
    </Tag>
  );
}