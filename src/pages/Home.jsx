    ...
    <div className="px-5 pt-6 pb-28 max-w-[430px] mx-auto">
      ...
      {cards.length > 0 ? (
            ...
            <button key={card.id} onClick={() => navigate('/accounts')} className="relative overflow-hidden rounded-[30px] p-6 min-h-[205px] w-full text-left text-white shadow-2xl bg-gradient-to-br from-[#0A2140] via-[#12355D] to-[#07162A]">
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-[9px] text-white/45 uppercase tracking-[0.14em] mt-1">Private Banking</p>
                </div>
              ...
              <div className="relative z-10 mt-7 flex justify-between">
                <div>
                  ...
                  <p className="text-xs mt-1 uppercase">{card.cardholder_name || firstName}</p>
                </div>
                <div>
                  <p className="text-[9px] text-white/45 uppercase">Type</p>
                  <p className="text-xs mt-1 capitalize">{card.card_type || 'debit'}</p>
                </div>
              </div>
            </button>
          ))}
        ...
        <button onClick={() => navigate('/services')} className="w-full text-left rounded-[30px] bg-gradient-to-br from-[#0A2140] to-[#193D68] text-white p-6 mb-7 shadow-xl">
          ...
          <h2 className="text-2xl font-semibold mt-2">Your card will live here</h2>
          <p className="text-sm text-white/65 mt-2">Request an eligible card and it will appear on your Home.</p>
      ...
      <section className="rounded-[28px] bg-white border border-border p-5 shadow-sm mb-5">
        ...
        <div className="mt-5 pt-4 border-t border-border/70 flex items-center justify-between text-sm">
          <span className="text-gray">{accounts.length} account{accounts.length === 1 ? '' : 's'}</span>
          <button onClick={() => navigate('/accounts')} className="font-semibold text-brass inline-flex items-center gap-1">Open Accounts <ChevronRight size={15} /></button>
        </div>
      </section>
      ...
      {wealth > 0 && (
        <section className="rounded-[28px] bg-[#0B2342] text-white p-5 shadow-xl mb-5">
          ...
          <button onClick={() => navigate('/accounts')} className="mt-5 text-sm font-semibold text-brass inline-flex items-center gap-1">View in Accounts <ChevronRight size={15} /></button>
        </section>
      )}

      <section className="grid grid-cols-2 gap-3">
        <button onClick={() => navigate('/accounts')} className="rounded-[24px] bg-white border border-border p-5 text-left shadow-sm">
          <Wallet size={21} className="text-brass" />
          <p className="font-semibold mt-4">Accounts</p>
          <p className="text-xs text-gray mt-1">Balances, cards, payments and transfers.</p>
        </button>
        <button onClick={() => navigate('/discovery')} className="rounded-[24px] bg-white border border-border p-5 text-left shadow-sm">
          <TrendingUp size={21} className="text-brass" />
          <p className="font-semibold mt-4">Discovery</p>
          <p className="text-xs text-gray mt-1">Verified opportunities, needs and intelligence.</p>
        </button>
      </section>
    </div>
  );
